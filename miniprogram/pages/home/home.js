...
function getServiceLabel(key) {
  ...
  for (index = 0; index < serviceTabs.length; index += 1) {
    if (serviceTabs[index].key === key) {
      return serviceTabs[index].label;
    }
  }

  return serviceTabs[0].label;
}
...
Page({
  data: {
    ...
    activeServiceLabel: getServiceLabel("cargo"),
    heroWord: cargoVehicleCard.heroWord,
    heroBadge: cargoVehicleCard.heroBadge,
    vehicleRows: cargoVehicleCard.rows,
    moreTransportLabel: "",
    transportTags: [],
    pickupBadge: "装",
  ...
  onServiceChange: function (event) {
    ...
    this.setData({
      ...
      activeServiceLabel: getServiceLabel(key),
    });
  },
